...
The threat model step adds PR title and PR description as inputs. Both are treated as untrusted: the PR description in particular is author-controlled text and must be assumed to contain adversarial content. The prompt loader wraps both in nonce-tagged XML delimiters before substitution so prompt instructions cannot be overridden by content inside the PR description field. The system prompt explicitly instructs the model to treat all supplied data, including PR title and description, as untrusted data rather than as instruction. Threat model output affects only the advisory PR comment and never the deterministic critical gate.
