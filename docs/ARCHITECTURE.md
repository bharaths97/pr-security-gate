...
`scanner/threat_model.py` reads `domain_context.json`, `terrain-findings.json`, and `triage-findings.json` plus the PR title and optional PR description passed from the GitHub event context. It makes one AI call via `scanner/ai_provider.py` and writes `threat-model.json` with blast radius, entry points added, up to three grounded STRIDE findings, mitigations present and absent, and domain-specific risks. Each STRIDE row carries evidence, why-it-applies, reviewer action, and confidence. If no provider key is configured, a required input file is missing, or the AI call fails, it writes `{"generated": false}` and exits 0. The threat model step is opt-in via the `run-threat-model` boolean input on the reusable workflow.
